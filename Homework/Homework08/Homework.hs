...
cabernet_sauvignon = "Cabernet-sauvignon" :: Grape
merlot = "Merlot" :: Grape
...
larrosaRose = Label{grapes=[garnacha],region = rioja, kind = Rose 14.0}
...
castiglioni = Label{grapes=[sangiovese],region=tuscany,kind= Red 12.5}
...
lePetitHaitLafitte = Label{grapes=[cabernet_sauvignon,merlot], region=bordeaux, kind= Red 13.5}
...
containsGrape ls g = any (\label -> elem g $ grapes label) ls
